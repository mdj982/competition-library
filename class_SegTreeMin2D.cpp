...
private:
	...
	val_t solve_rec(int s0, int t0, int s1, int t1, int l, int r, int id) {
		...
		if (s0 == l && t0 == r) {
			...
			v = nodes[id].st->minof(s1_code, t1_code);
		}
		else {
			int m = (l + r) >> 1;
			if (s0 < m && m < t0) {
...
public:
	...
	SegTreeMin2D(int n, const vector<P> &id_pairs, const vector<val_t> &a) {
		struct pt_t {
			int row_id;
			ll code;
			val_t val;
		...
		Loop(i, base + N) {
			nodes[i].st = new SegTreeMin(as[i]);
		}
	}
