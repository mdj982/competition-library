...
private:
	...
	void change_rec(int s0, int t0, int s1, int t1, int l, int r, int id, val_t x, change_t op) {
		if (op == UPD) nodes[id].st->upd(nodes[id].encoder.lower_bound(s1)->snd, nodes[id].encoder.lower_bound(t1)->snd, x * (t0 - s0));
		if (op == ADD) nodes[id].st->add(nodes[id].encoder.lower_bound(s1)->snd, nodes[id].encoder.lower_bound(t1)->snd, x * (t0 - s0));
		if (s0 == l && t0 == r) {
			return;
		}
		else {
	...
	val_t solve_rec(int s0, int t0, int s1, int t1, int l, int r, int id) {
		val_t v = 0;
		if (s0 == l && t0 == r) {
			v = nodes[id].st->sumof(nodes[id].encoder.lower_bound(s1)->snd, nodes[id].encoder.lower_bound(t1)->snd);
		}
		else {
			int m = (l + r) >> 1;
			if (s0 < m && m < t0) {
...
public:
	...
	SegTreeSum2D(int n, const vector<P> &id_pairs) {
		this->n = n;
		N = 1 << ceillog2(n);
		base = N - 1;
		common_init();
		...
		Loop(i, base + N) {
			...
			nodes[i].st = new SegTreeSum(nodes[i].n);
		}
	}
