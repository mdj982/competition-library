...
private:
		...
		Loop1(ki, log_n - 1) {
			...
			Loop(i, n - di) {
				Loop(j, m) {
					...
					val = std::min(val, table[ki - 1][0][i][j]);
					val = std::min(val, table[ki - 1][0][i + (di >> 1)][j]);
				}
			}
		}
		Loop1(kj, log_m - 1) {
			...
			Loop(ki, log_n) {
				...
				Loop(i, n - di) {
					Loop(j, m - dj) {
						...
						val = std::min(val, table[ki][kj - 1][i][j]);
						val = std::min(val, table[ki][kj - 1][i][j + (dj >> 1)]);
					}
				}
			}
		}
	...
	void make_floorlogs() {
		floorlogs.resize(std::min(n, m) + 1);
		int p = 0;
...
public:
	...
	ll minof(int s0, int t0, int s1, int t1) {
		...
		int dj = 1 << kj;
		ll ret = LLONG_MAX;
		ret = std::min(ret, table[ki][kj][s0][s1]);
		ret = std::min(ret, table[ki][kj][t0 - di][s1]);
		ret = std::min(ret, table[ki][kj][s0][t1 - dj]);
		ret = std::min(ret, table[ki][kj][t0 - di][t1 - dj]);
		return ret;
	}
};